...
try {
  ...
  if (-not $NoShortcuts) {
    ...
    foreach ($folder in @($desktop, $startMenu)) {
      $shortcut = $shell.CreateShortcut((Join-Path $folder 'Moonlit Cards Dream Skin.lnk'))
      $shortcut.TargetPath = $powershell
      $shortcut.Arguments = "-NoProfile -ExecutionPolicy Bypass -File `"$startScript`"$portArgument -PromptRestart"
      $shortcut.WorkingDirectory = $SkillRoot
      $shortcut.Description = 'Launch Codex with the unofficial Moonlit Cards theme'
      $shortcut.Save()
    }

    $restore = $shell.CreateShortcut((Join-Path $desktop 'Moonlit Cards Dream Skin - Restore.lnk'))
    ...
    $restore.Description = 'Restore the standard Codex appearance'
    $restore.Save()

    foreach ($folder in @($desktop, $startMenu)) {
      $tray = $shell.CreateShortcut((Join-Path $folder 'Moonlit Cards Dream Skin - Tray.lnk'))
      ...
      $tray.Description = 'Open Moonlit Cards theme controls in the system tray'
      $tray.Save()
    }
    $autoApply = $shell.CreateShortcut((Join-Path $startup 'Moonlit Cards Dream Skin Auto Apply.lnk'))
    $autoApply.TargetPath = $powershell
    $autoApply.Arguments = "-NoProfile -STA -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$trayScript`"$portArgument"
    $autoApply.WorkingDirectory = $SkillRoot
    $autoApply.Description = 'Keep the unofficial Moonlit Cards theme active after Codex updates'
    $autoApply.Save()
  ...
  if ($NoShortcuts) {
    Write-Host 'Moonlit Cards base theme installed. Run start-dream-skin.ps1 to launch it.'
  } else {
    Write-Host 'Moonlit Cards Dream Skin installed with automatic reapply enabled.'
  }
} finally {
