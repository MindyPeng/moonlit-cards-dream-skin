...
try {
  ...
  if ($initialTheme.Theme.id -cne 'moonlit-cards-original' -or
    $initialTheme.Theme.name -cne 'Moonlit Cards Dream Skin' -or
    ...
    throw 'Default Windows theme did not seed the Moonlit Cards wallpaper contract.'
  }
  $preseededThemes = @(Get-DreamSkinSavedThemes -StateRoot $themeStateRoot)
  if ($preseededThemes.Count -ne 1 -or
    $preseededThemes[0].Id -cne 'moonlit-cards-original' -or
    $preseededThemes[0].Name -cne 'Moonlit Cards Dream Skin') {
    throw 'Moonlit Cards was not preseeded in the Windows saved-theme menu.'
  }
  $updatedTheme = Set-DreamSkinActiveTheme -ImagePath (Join-Path $Root 'assets\moonlit-cards-original.png') `
  ...
  Copy-Item -LiteralPath (Join-Path $Root 'assets\moonlit-cards-original.png') `
    -Destination (Join-Path $outsideTheme 'moonlit-cards-original.png')
  ...
  foreach ($requiredTrayAction in @('System.Windows.Forms.NotifyIcon', '暫停皮膚', '更換背景圖', '已儲存主題', '完全還原 Codex')) {
  ...
  foreach ($requiredAutoApply in @('$script:DreamSkinLastAutoApplySignature', '$autoTimer.Interval = 5000', 'TotalSeconds -lt 30', "'Moonlit Cards Dream Skin Auto Apply.lnk'")) {
    $source = if ($requiredAutoApply -like '*Auto Apply*') {
      Read-DreamSkinUtf8File -Path (Join-Path $Root 'scripts\install-dream-skin.ps1')
    } else {
      $traySource
    }
    if (-not $source.Contains($requiredAutoApply)) { throw "Automatic reapply is missing: $requiredAutoApply" }
  }
  ...
  if ($theme.schemaVersion -ne 1 -or $theme.id -cne 'moonlit-cards-original' -or
    $theme.name -cne 'Moonlit Cards Dream Skin' -or $theme.image -cne 'moonlit-cards-original.png') {
    throw 'The public theme manifest is incorrect.'
  }
  if (-not (Test-Path -LiteralPath (Join-Path $Root 'assets\moonlit-cards-original.png'))) {
    throw 'The original public wallpaper is missing.'
  }
  foreach ($entry in @('Install.cmd', 'Uninstall.cmd', 'LICENSE', 'NOTICE.md', 'README.md', 'README.en.md',
    'ARTWORK.md', 'VERSION', 'scripts\install-package.ps1', 'scripts\uninstall-package.ps1',
    'scripts\build-windows-release.ps1')) {
    if (-not (Test-Path -LiteralPath (Join-Path $ProjectRoot $entry))) { throw "Public package entry is missing: $entry" }
  }
  foreach ($scriptFile in Get-ChildItem -LiteralPath $ProjectRoot -Recurse -Filter '*.ps1' -File) {
    ...
    if ($parseErrors.Count -gt 0) { throw "PowerShell parse error in $($scriptFile.FullName): $($parseErrors[0].Message)" }
  }
