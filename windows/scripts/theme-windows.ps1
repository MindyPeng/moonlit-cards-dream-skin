...
function Get-DreamSkinThemePaths {
  param([string]$StateRoot = (Join-Path $env:LOCALAPPDATA 'MoonlitCardsDreamSkin'))
...
function Initialize-DreamSkinThemeStore {
  param(
    ...
    [string]$StateRoot = (Join-Path $env:LOCALAPPDATA 'MoonlitCardsDreamSkin')
  )
  ...
  $assetImageName = 'moonlit-cards-original.png'
  ...
  $presetDirectory = Join-Path $paths.Saved 'moonlit-cards-original'
